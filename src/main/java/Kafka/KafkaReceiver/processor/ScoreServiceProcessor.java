...
//    @Value("${spring.kafka.window-size}")
    ...
    public void processChanges(StreamsBuilder streamsBuilder){
        ...
        KTable<Windowed<String>, String> windowedStreamX = streamsBuilder.stream("cdx-1", Consumed.with(Serdes.String(), Serdes.String()))
                .groupByKey()
                .windowedBy(TimeWindows.of(windowSize))
                .reduce((v1, v2) -> {
                    int val =  Integer.parseInt(v1) + Integer.parseInt(v2);
                    return val + "";
                }).toStream().peek((k,v)-> System.out.println("kx: "+k+" vx:"+ v)).toTable();

        KTable<Windowed<String>, String> windowedStreamY = streamsBuilder.stream("cdx-2", Consumed.with(Serdes.String(), Serdes.String()))
                .groupByKey()
                .windowedBy(TimeWindows.of(windowSize))
                .reduce((v1, v2) -> {
                    int val =  Integer.parseInt(v1) + Integer.parseInt(v2);
                    return val + "";
                }).toStream().peek((k,v)-> System.out.println("ky: "+k+" vy:"+ v)).toTable();
        ...
        windowedStreamX
                ...
                .map((k, v) -> {
                    ...
                    System.out.println("weightedInteraction: "+ interaction);
                ...
                .map((k, v) -> {
                    System.out.println("SI Value: " + v + " Key: "+ k.key());
